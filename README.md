...
### ✅ IO按钮防抖和状态检测优化 + 服务重启修复

**核心改进**：优化IO按钮防抖逻辑，区分按钮IO和开关IO，增强状态检测，修复服务重启问题

#### 1. IO按钮防抖机制 ✅
- **按钮防抖实现**：为启动、停止、复位按钮添加200ms防抖时间
  - 防止按钮弹跳导致的重复触发
  - 使用时间戳检测，只有距离上次触发超过200ms的按键才会生效
  - 防抖被过滤的事件会记录Debug级别日志，便于调试
- **IO类型区分**：
  - **按钮类型IO**（启动、停止、复位）：使用边沿检测+防抖，只在按下瞬间触发一次
  - **开关类型IO**（急停、远程/本地）：使用电平检测，持续监测状态变化
- **修改文件**：
  - `LeadshineSafetyIoModule.cs` - 新增防抖时间戳、更新轮询逻辑
...
|--------|---------|---------|---------|---------|
| **按钮IO** | 瞬时接通，松开断开 | 边沿检测（上升沿） | 200ms防抖 | 启动、停止、复位按钮 |
