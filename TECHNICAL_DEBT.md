...
2. 将 `SafeOperationHelper` 重构为使用 `ICabinetIsolator`
3. 更新所有调用点
4. 添加迁移指南文档

**相关文件**:
- `ZakYip.Singulation.Infrastructure/Runtime/SafeOperationIsolator.cs`
- `ZakYip.Singulation.Infrastructure/Cabinet/CabinetIsolator.cs`
- `ZakYip.Singulation.Host/SwaggerOptions/SafeOperationHelper.cs`

**验证标准**:
- [ ] SafeOperationIsolator 完全移除
- [ ] SafeOperationHelper 使用 ICabinetIsolator
- [ ] 所有调用点更新完成
- [ ] 单元测试通过
- [ ] 运行 `tools/check-duplication.sh` 显示≤2处
