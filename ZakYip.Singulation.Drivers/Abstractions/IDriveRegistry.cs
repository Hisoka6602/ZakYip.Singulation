...
namespace ZakYip.Singulation.Drivers.Abstractions {
    ...
    public interface IDriveRegistry {
        ...
        /// <summary>
        /// 驱动工厂注册表：按厂商标识（vendor）注册 <see cref="IAxisDrive"/> 的构造工厂，
        /// 并在运行期基于轴ID、端口与选项创建具体驱动实例。
        /// </summary>
        /// <remarks>
        /// 约定与建议：
        /// <list type="bullet">
        /// <item>实现应对 <c>vendor</c> 使用大小写不敏感的键；</item>
        /// <item><see cref="Register"/> 可重复调用，后者覆盖同名厂商（覆盖策略由实现说明）；</item>
        /// <item><see cref="Create"/> 返回的实例归调用方所有，调用方负责释放（见 <see cref="IAsyncDisposable"/>）；</item>
        /// <item>在并发环境中，注册表实现应保证线程安全（尤其是多线程创建/读取）。</item>
        /// </list>
        /// </remarks>
        public interface IDriveRegistry {
