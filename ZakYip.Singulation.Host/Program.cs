...
try {
    ...
    if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
        ...
        // ---------- Windows 系统配置（使用统一的安全隔离器，异步执行不阻塞主线程） ----------
        var cabinetIsolator = host.Services.GetRequiredService<ICabinetIsolator>();
        var loggerFactory = host.Services.GetRequiredService<ILoggerFactory>();

        // 异步执行 Windows 配置，不阻塞主线程
        _ = Task.Run(async () => {
            // ---------- 配置 Windows 防火墙 ----------
            await cabinetIsolator.SafeExecuteAsync(async () => {
                ...
                // 检查并配置防火墙（同步操作，包装为 Task）
                await Task.Run(() => firewallManager.CheckAndConfigureFirewall(applicationPath, applicationName, ports)).ConfigureAwait(false);
            }, 
            "Windows防火墙配置",
            ...
            await cabinetIsolator.SafeExecuteAsync(async () => {
                ...
                // 配置所有网卡：启用巨帧、最大化传输缓存、禁用节能（同步操作，包装为 Task）
                await Task.Run(() => networkManager.ConfigureAllNetworkAdapters()).ConfigureAwait(false);
            },
            "网络适配器配置",
