...
### 更新内容
- **默认轴布局兜底**：`MainViewModel` 初始化时即构建 20 个占位轴（M01-M20），并在 API 返回空数据或异常时自动恢复默认布局，确保界面始终展示完整的轴网格。
- **轴信息模型增强**：`AxisInfo` 新增 `CurrentRpm` 与 `IsPlaceholder` 字段，用于区分实时数据和占位数据，同时补齐 UI 所需的 r/min 速度显示。
- **顶部操作卡片焕新**：`MainPage.xaml` 顶部操作区重构为四个带图标的卡片按钮（刷新控制器、安全指令、全部使能、输速设定），结合颜色与开关状态呈现更贴近设计稿的交互体验。

### 受影响的文件结构
```
ZakYip.Singulation.MauiApp/
├── MainPage.xaml                # 顶部操作区卡片化样式与说明文案焕新
├── Services/
│   └── ApiClient.cs             # AxisInfo 扩展 CurrentRpm 与 IsPlaceholder 字段
└── ViewModels/
    └── MainViewModel.cs         # 默认轴布局兜底逻辑与操作状态文案/命令更新
```

### 可继续完善的内容
- 默认轴数据可接入本地缓存，优先展示最近一次成功拉取的实时数据。
- 顶部操作卡片可替换为 FontIcon 资源，保证跨平台图标统一。
- `CurrentRpm` 与线速度（mm/s）之间的换算公式可根据电机参数进一步精细化。
...
```
ZakYip.Singulation.MauiApp/
├── MainPage.xaml                # 分件助手主页卡片化布局与弹出面板
└── ViewModels/
    └── MainViewModel.cs         # 自动刷新、全局使能、面板切换等新状态逻辑
```

### 可继续完善的内容
- 自动刷新可增加自定义时间间隔与倒计时提示。
- 主页卡片可根据轴状态自动配色，并支持按状态筛选。
- 安全指令面板可引入命令历史与最近原因快捷选择。
