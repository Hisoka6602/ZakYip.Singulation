...
namespace ZakYip.Singulation.Host.Controllers {
    ...
    /// 通过删除当前会话资源来触发宿主退出并重启，交由外部进程（Windows 服务管理器）负责重启。
    /// </summary>
    [ApiController]
    ...
    public sealed class SystemSessionController : ControllerBase {
        ...
        /// </summary>
        /// <remarks>
        /// 删除当前运行会话资源，触发宿主应用退出并重启。
        /// 此操作会通知宿主停止运行并以非零退出码退出进程，触发 Windows 服务管理器自动重启服务。
        /// 外部部署工具（如 Windows 服务管理器或 systemd）应配置为自动重启服务。
        ///
        /// 注意：此操作是异步执行的，API 会立即返回 202 状态码，实际退出和重启会在后台进行。
        /// </remarks>
        /// <param name="ct">取消令牌</param>
        /// <returns>操作受理结果</returns>
        /// <response code="202">关闭请求已受理，服务正在准备退出并重启</response>
        /// <response code="400">请求已取消</response>
        [HttpDelete]
        [SwaggerOperation(
            Summary = "删除当前运行会话",
            Description = "删除当前运行会话资源，触发宿主应用退出并重启。此操作会通知宿主停止运行并以非零退出码退出进程，触发 Windows 服务管理器自动重启服务。注意：此操作是异步执行的，API 会立即返回 202 状态码，实际退出和重启会在后台进行。")]
        ...
        public ActionResult<ApiResponse<object>> DeleteCurrentSession(CancellationToken ct) {
            ...
            // 中文日志：收到关闭请求
            _logger.LogInformation("收到关闭请求，将在后台停止并重启宿主应用。");
            ...
            _ = Task.Run(async () => {
                try {
                    ...
                    _logger.LogInformation("执行强制退出，退出码=1，触发服务重启。");
                    Environment.Exit(1);
                }
                catch (Exception ex) {
                    // 中文日志：异常兜底，保证非零退出
            ...
            // 立即返回 202，提示后台正在处理
            return Accepted(ApiResponse<object>.Success(new { Accepted = true }, "服务正在准备退出并重启"));
        }
    }
}