...
function show_stats() {
    ...
    # 统计各优先级数量（仅统计待处理的）
    P0_COUNT=$(awk '
        /^### TD-[0-9]+:/ {in_td=1; is_p0=0; is_pending=0; next}
        in_td && /\*\*优先级\*\*: P0/ {is_p0=1}
        in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
        /^### / && in_td {
            if(is_p0 && is_pending) count++
            in_td=0
        }
        END {
            if(in_td && is_p0 && is_pending) count++
            print count+0
        }
    ' "$TECH_DEBT_FILE")
    
    P1_COUNT=$(awk '
        /^### TD-[0-9]+:/ {in_td=1; is_p1=0; is_pending=0; next}
        in_td && /\*\*优先级\*\*: P1/ {is_p1=1}
        in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
        /^### / && in_td {
            if(is_p1 && is_pending) count++
            in_td=0
        }
        END {
            if(in_td && is_p1 && is_pending) count++
            print count+0
        }
    ' "$TECH_DEBT_FILE")
    
    P2_COUNT=$(awk '
        /^### TD-[0-9]+:/ {in_td=1; is_p2=0; is_pending=0; next}
        in_td && /\*\*优先级\*\*: P2/ {is_p2=1}
        in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
        /^### / && in_td {
            if(is_p2 && is_pending) count++
            in_td=0
        }
        END {
            if(in_td && is_p2 && is_pending) count++
            print count+0
        }
    ' "$TECH_DEBT_FILE")
    
    P3_COUNT=$(awk '
        /^### TD-[0-9]+:/ {in_td=1; is_p3=0; is_pending=0; next}
        in_td && /\*\*优先级\*\*: P3/ {is_p3=1}
        in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
        /^### / && in_td {
            if(is_p3 && is_pending) count++
            in_td=0
        }
        END {
            if(in_td && is_p3 && is_pending) count++
            print count+0
        }
    ' "$TECH_DEBT_FILE")
    
    # 统计各状态数量
    PENDING_COUNT=$(grep -c "\*\*状态\*\*: ⏳ 待处理" "$TECH_DEBT_FILE" 2>/dev/null || echo "0")
    IN_PROGRESS_COUNT=$(grep -c "\*\*状态\*\*: 🔄 进行中" "$TECH_DEBT_FILE" 2>/dev/null || echo "0")
    DONE_COUNT=$(grep -c "\*\*状态\*\*: ✅ 已完成" "$TECH_DEBT_FILE" 2>/dev/null || echo "0")
...
function show_pending() {
    ...
    awk '
        /^### TD-[0-9]+:/ {
            if(in_td && is_pending) print title
            in_td=1; is_pending=0; title=$0
        }
        in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
        END {
            if(in_td && is_pending) print title
...
function show_p0() {
    ...
    if [ "$P0_COUNT" -eq 0 ]; then
        ...
        awk '
            /^### TD-[0-9]+:/ {
                if(in_td && is_p0 && is_pending) print title
                in_td=1; is_p0=0; is_pending=0; title=$0
            }
            in_td && /\*\*优先级\*\*: P0/ {is_p0=1}
            in_td && /\*\*状态\*\*: ⏳ 待处理/ {is_pending=1}
            END {
